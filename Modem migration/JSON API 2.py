...
class mobilityParser(object):
    ...
    def getModem(self,modemCount):
        ...
        modemPagecount =  modemCount['count']//100
        leftModemCount = modemCount['count']%100
        modemList = []
        for modemPage in range(modemPagecount+1):
            start = (modemPage * 100) +1
            if modemPage == (modemPagecount + 1):
                modem = rest.GET('modem?limit=100&start={}'.format(start))
                modemList.append([(i['remoteGuiUrl'],i['macAddress'],i['id']['domainName'],i['id']['name'],i['locked']) for i in modem])
            else:
                modem = rest.GET('modem?limit=100&start={}'.format(start))
                modemList.append([(i['remoteGuiUrl'],i['macAddress'],i['id']['domainName'],i['id']['name'],i['locked']) for i in modem])
        return modemList
